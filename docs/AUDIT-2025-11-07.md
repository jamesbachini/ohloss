...
- ✅ **Persistent Storage**: Player data, epoch data, game whitelist (all keyed by Address/u32)
...
**Player Functions**:
```rust
// vault.rs:43 - Deposit requires player auth
pub fn deposit(env: Env, player: &Address, amount: i128) -> Result<(), Error> {
    player.require_auth();  // ✅ Protected
    // ...
}

// vault.rs:123 - Withdraw requires player auth
pub fn withdraw(env: Env, player: &Address, amount: i128) -> Result<(), Error> {
    player.require_auth();  // ✅ Protected
    // ...
}

// rewards.rs:38 - Claim requires player auth
pub fn claim_yield(env: &Env, player: &Address, epoch: u32) -> Result<i128, Error> {
    player.require_auth();  // ✅ Protected
    // ...
}
```
...
- ✅ All player state-changing functions: `deposit`, `withdraw`, `select_faction`, `start_game`, `claim_yield`
...
```rust
// vault.rs:191-198
let threshold = epoch_player
    .initial_epoch_balance
    .fixed_mul_floor(WITHDRAWAL_RESET_THRESHOLD, SCALAR_7)  // 50%
    .ok_or(Error::OverflowError)?;

let reset = epoch_player.withdrawn_this_epoch > threshold;

if reset {
    player_data.deposit_timestamp = env.ledger().timestamp();  // Reset time
    let new_fp = calculate_faction_points(env, player)?;       // Recalculate FP
    epoch_player.available_fp = new_fp.saturating_sub(epoch_player.locked_fp);
}
```

**Security Properties**:
- ✅ Tracks `withdrawn_this_epoch` per player

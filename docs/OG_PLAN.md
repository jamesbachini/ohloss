...
select_faction(player, faction)
	...
	Do not allow a player to select a faction after the epoch has started unless it is their first action for the epoch (hasn’t played any games yet)
		This means we both track a per player and a per epoch faction
		This might mean the easier thing to do will be to allow this method to be called at any time but once the first game for the player is played they lock in their epoch faction at that time

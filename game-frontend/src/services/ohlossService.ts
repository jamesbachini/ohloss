/**
 * Ohloss Service - Read-only contract data access
 *
...
const OHLOSS_CONTRACT = import.meta.env.VITE_OHLOSS_CONTRACT || ''
...
let configCache: { config: OhlossConfig; fetchedAtMs: number } | null = null

function parseInstanceStorage(data: xdr.LedgerEntryData): { epoch: number | null; config: OhlossConfig | null } {
  try {
 ...
 */
async function fetchInstanceStorage(): Promise<{ epoch: number; config: OhlossConfig | null }> {
  if (!OHLOSS_CONTRACT) return { epoch: 0, config: null }
  ...
  try {
    ...
    const instanceLedgerKey = storageKeyToLedgerKey(OHLOSS_CONTRACT, xdr.ScVal.scvU32(0), 'instance')
 ...
 */
async function getConfig(): Promise<OhlossConfig | null> {
...
async function getPlayerData(playerAddress: string): Promise<{ timeMultiplierStart: bigint } | null> {
  ...
  try {
    ...
    const ledgerKey = storageKeyToLedgerKey(OHLOSS_CONTRACT, playerKey, 'persistent')
...
export async function getAvailableFp(playerAddress: string): Promise<bigint> {
  if (!OHLOSS_CONTRACT) return DEFAULT_FREE_FP
...
export async function hasFactionSelected(playerAddress: string): Promise<boolean> {
  if (!OHLOSS_CONTRACT) return false

  try {
    const rpcClient = getRpc()

    const playerKey = buildPlayerKey(playerAddress)
    const playerLedgerKey = storageKeyToLedgerKey(OHLOSS_CONTRACT, playerKey, 'persistent')
...
export async function getGamePageData(playerAddress: string): Promise<{
  ...
  try {
    ...
    if (cachedEpoch !== null) {
      ...
      let config: OhlossConfig | null = configCache?.config ?? null

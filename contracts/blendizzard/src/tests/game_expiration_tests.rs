...
fn test_games_expire_on_epoch_cycle() {
    ...
    // Setup two players
    blendizzard.select_faction(&p1, &0); // WholeNoodle
    blendizzard.select_faction(&p2, &1); // PointyStick
    ...
    let p1_epoch0 = blendizzard.get_epoch_player(&blendizzard.get_current_epoch(), &p1);
    let p2_epoch0 = blendizzard.get_epoch_player(&blendizzard.get_current_epoch(), &p2);
    ...
    let p1_epoch1 = blendizzard.get_epoch_player(&blendizzard.get_current_epoch(), &p1);

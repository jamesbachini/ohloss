...
fn test_withdrawal_reset_50_percent_exactly() {
    ...
    let (game_contract, _vault_addr, mock_vault, blendizzard) = setup_math_test_env(&env);
...
fn test_fp_calculation_rounds_down() {
    ...
    let epoch_player2 = blendizzard.get_epoch_player(&0, &player1);
    assert_eq!(
...
fn test_reward_calculation_rounds_down() {
    ...
    blendizzard.select_faction(&p2, &0);
    blendizzard.select_faction(&p3, &0);
    ...
    // Claim rewards
    let r1 = blendizzard.claim_epoch_reward(&p1, &0);
    let r2 = blendizzard.claim_epoch_reward(&p2, &0);
    let r3 = blendizzard.claim_epoch_reward(&p3, &0);
